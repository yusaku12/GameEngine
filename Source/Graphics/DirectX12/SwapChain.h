...
namespace Engine
{
	...
	public:
		 ...
		 */
		[[nodiscard]] DX12Resource* getCurrentBackBuffer() noexcept;

		/**
		 * @brief 現在の Back Buffer RTV を取得する
		 * @return 現在の Back Buffer RTV
		 */
		[[nodiscard]] DX12CpuDescriptorHandle getCurrentRtv() const noexcept;

		/**
		 * @brief 現在の Back Buffer Index を取得する
		 * @return 現在の Back Buffer Index
		 */
		[[nodiscard]] std::uint32_t getCurrentBackBufferIndex() const noexcept { return m_currentBackBufferIndex; }
